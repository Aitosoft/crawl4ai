...
class StealthAdapter(BrowserAdapter):
    ...
    Aitosoft 2026-04-11: patched to use playwright-stealth 2.x API. Upstream
    crawl4ai v0.8.6 pins playwright-stealth>=2.0.0 in pyproject.toml but this
    class still used the 1.x import names (stealth_async/stealth_sync), which
    no longer exist in 2.x. The old code failed the imports silently and
    became a no-op, so enable_stealth=True had no effect. The 2.x API is
    class-based: Stealth().apply_stealth_async(page).
    """
    ...
    def _check_stealth_availability(self) -> bool:
        """Check if playwright_stealth 2.x is available and instantiate Stealth."""
        try:
    ...
    async def apply_stealth(self, page: Page):
        """Apply stealth to a page if available (best-effort, failures are swallowed)."""
        if self._stealth_available and self._stealth_instance is not None:
            try:
                await self._stealth_instance.apply_stealth_async(page)
            except Exception:
                # Fail silently — stealth is best-effort. A broken patch
                # should not take down the crawl.
                pass

        # Aitosoft 2026-04-11: playwright-stealth's navigator.webdriver.js
        # patch only activates when `navigator.webdriver` is TRUTHY. Our
        # --disable-blink-features=AutomationControlled flag makes it return
        # `false` (falsy but still defined), which bypasses the stealth patch
        # AND leaks automation to any detector checking property presence
        # (Cloudflare, creepjs, bot.sannysoft.com etc). Real browsers have
        # `navigator.webdriver === undefined`. Force it via init script so
        # every future navigation sees the real-Chrome shape.
        try:
