...
| OOM / MemoryError "refusing new browser" | **Our pool guard**, not OS-OOM. Replica hit ~85%+ and refused a new browser spawn. | Peek pool mem% timeline (`Pool: hot=… mem=…` log lines). If it drops back within ~5 min, no action — the guard worked. ~~If it sticks >85% for 10+ min, restart the revision.~~ **DO NOT restart on this alone — corrected 2026-08-10.** The overnight sweep sat above 85 % for ~15 hours and a restart would have been wrong every time: at 420 refusal events the guard read **88.7 %** while true `anon` was **68.6 %**, the rest being **583 MB of reclaimable active page cache**. **Read `anon=` in the same line, not the percentage.** ⛔ **"Escalate on `anon` ≥85 %, an `OOMKilled`, or an exit 137 — none of which have ever been seen" was true until 2026-08-15 04:30, when an `OOMKilled` / exit 137 happened.** It self-recovered in seconds, cost zero pages, and — the part that matters for this row — **it fired at ordinary readings**: `anon` was 2,835–3,445 MB and `mem` 80.5–93.5 % in the 90 s before, i.e. the middle of the all-night band. **So neither number predicts a kill and you cannot escalate on a memory reading at all.** What you *can* do is detect the kill after the fact (see the restart row below) and check whether it cost pages. `tasks/memory-guard-charges-reclaimable-page-cache.md` |
| **A container can restart WITHOUT the replica changing, and `az containerapp replica list` cannot see it** | Measured 2026-08-14/15: four container restarts (19:33, 21:11, 04:23 ×2 → the 04:30 OOM). The replica object survives, `createdTime` never changes, and three of the four reported **no reason string at all**. A monitoring session checking the replica list sees two healthy replicas and concludes nothing happened — which is exactly what happened to me twice before I caught it | **Detect it directly:** `ContainerAppSystemLogs_CL \| where Reason_s == "ContainerStarted"`, then cross-check each timestamp against replica `createdTime` — any `ContainerStarted` **not** within ~10 s of a replica creation is a silent restart. Confirm with console `supervisord started with pid 1`. **Always look for the `SuccessfulRescale` scale-down/scale-up pair before treating one as a crash** — ordinary scale churn produces the same `ContainerStarted` line |
...
- ⛔ ~~Memory alert `Fired` (monitorCondition ≠ null/Resolved) → restart the
  ...
  an intervention table. **Also: nothing documents which metric that alert reads,
  and it is NOT `get_container_memory_percent`** — establish that before acting on
  it at all. On a memory alert: check `anon=`, check for an actual `OOMKilled` /
  exit 137, and check whether pages were lost. Restart only if pages are being
  lost *now*.
- Replica non-Running >10 min → restart. **But see the silent-restart row above:
  the replica list is blind to a container restart, so "all replicas Running" is
  not evidence that nothing crashed.** A service can OOM, die and recover with the
  replica list unchanged throughout.
