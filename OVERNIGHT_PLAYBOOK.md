...
| RENDER-DEFECT ("RENDER DEFECT: url=… content collapsed in our parse: N chars of visible text …, 0 chars of markdown out") | Our parse lost the whole body. Served as **HTTP 200 + result `success:false` + `failure_class: render_defect`**, content attached, zero retries. Deployed `0.9.2-detector-round3` 2026-08-01. | **Record the URLs — do not intervene.** It is already the correct outcome for a page we cannot parse, and no restart or rollback changes it. Baseline is **2.7 % of pages / 18 % of hosts** (2026-08-01, 9 URLs of 328). Worth a note to Tero if the rate moves materially either way, since it is the metric `tasks/cleaned-html-collapse-guard.md` item 1 is meant to drive toward zero. **MAS cannot see this class at all** — a page with no contacts looks like a page that has no contacts — so their "all clean" is not a cross-check. |
