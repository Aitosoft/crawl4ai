...
async def test_anti_bot_site(use_stealth: bool = False) -> Dict[str, Any]:
    ...
    browser_config = BrowserConfig(
        ...
        if not use_stealth
        else [],  # These are automatically applied with stealth
    )
...
async def stealth_best_practices():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        # First request
        await crawler.arun(url="https://example.com", config=config)

        # Subsequent request reuses the same browser context
        await crawler.arun(url="https://example.com/about", config=config)
