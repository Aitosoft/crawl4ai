...
- **Image:** `aitosoftacr.azurecr.io/crawl4ai-service:0.9.2-desc-cap`, **revision `--0000042`** (image unchanged since 2026-08-09; `--0000042` is the 2026-08-14T15:59:42Z **scale-rule** change, not a new build). ⛔ **This line pointed at `AITOSOFT_CHANGES.md` "Current State" as authoritative and that section was a revision behind — so both were wrong and the pointer sent you to the worse one.** `az containerapp show --query properties.template` beats every document here; trust it over all of them.
- **Resources:** 2 vCPU / 4 GiB per replica, **0-20 replicas** (MAS lowered it 45 -> 20 on 2026-08-14 as tail protection; observed peak across the whole sweep was 19, so it does not bind and saved exactly EUR 0). Scale rule `http-renders` trigger is **12 since 2026-08-14** (was 6). **What that actually produces: a flat 2-replica fleet on MAS's full sweep load, and it is only weakly elastic** -- one scale-up event in 30 minutes of real traffic, and a synthetic arm at 1.44x their rate held 1 replica for 12 minutes through 38 rejections. `maxReplicas` is therefore no longer a lever; the levers are the trigger and `minReplicas`. **The hard ceiling is 50** — environment quota `ManagedEnvironmentConsumptionCores` is 100 and we are 2 vCPU/replica (`az containerapp env list-usages`); MAS's `aitosoft-edge` shares the environment at 0.25 cores. The scale rule "MUST match `render_capacity`" claim is **settled and gone** — it was a category error (different quantities, different units), the trigger is **12** since 2026-08-14, and `deploy-image.sh` now checks the live rule against its own `ACA_SCALE_TRIGGER` constant instead of against `render_capacity`. **2 vCPU / 4 GiB is this environment's hard maximum** — it is a legacy Consumption-only managed environment and Azure rejects anything larger; more memory needs an environment migration with a different billing model, not a resize (2026-08-02, tested).
