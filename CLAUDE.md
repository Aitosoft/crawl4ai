...
| **The ACA scale trigger is not `render_capacity`, and pinning them was a category error** | `render_capacity` is a hard in-process cap on **concurrent renders** (the safety mechanism; RenderGate enforces it). ACA's `concurrentRequests` is only **when Azure adds a replica**, and Microsoft's own doc defines it as *"requests in the past 15 seconds divided by 15"* — a **rate**, while naming it concurrency. Two numbers in different units can never meaningfully be equal. The old "MUST match" comment made the trigger maximally twitchy. **Trigger is 6 since 2026-08-08** (ACA's own default is 10; we were at 2). Raising it cannot oversubscribe a replica. `deploy-image.sh` now checks live-vs-`ACA_SCALE_TRIGGER`, not live-vs-`render_capacity` — and it **hard-fails `exit 1`**, so a scale-rule change made without editing that constant breaks the next deploy. ⛔ **This row said the check runs *after* the image is already updated — true until 2026-08-14, false since.** The drift checks were moved *before* the build (`deploy-image.sh:65-78`), precisely because failing after the update is the worst of both: a red deploy and a changed production. **A drift failure now means nothing was built and nothing shipped** — the opposite of what this row promised, and the difference decides whether you go looking for a half-deployed service |
...
| ⛔ **"Over-provisioning cost is €0 cash" is WRONG and it is why ~$89/day ran unnoticed for five days** — and **the currency was wrong too** | The subscription is **Sponsored** (`quotaId: Sponsored_2016-01-01`), which Microsoft documents **by that exact quota ID** in Cost Management's unsupported-offer list, so the API returns `rows: []`. **"The API returned no records" and "there was no cost" are different statements**, and reading the first as the second is what let this run for five days. ⛔ **Every euro figure this repo published before 2026-08-17 is ~12 % high**: the usage export is denominated in **USD** and carries **no currency column**. Verified against `prices.azure.com` on three meters — Azure's EUR list is **0.8775 × USD**. So `€398.89 for one sweep` is **$398.89 ≈ €350**, and `~€89/day → ~€14.5/day` is **~$89 → ~$14.5/day ≈ €78 → €12.7**. The measured rate is **$0.000034/vCPU-s + $0.000004/GiB-s = $0.3024/replica-hour** at 2 vCPU / 4 GiB. **Also correct the instrument**: there is **no configured cost export** (`Microsoft.CostManagement/exports` → `{"value": []}`) — the figure came from a portal download Tero placed in gitignored `tmp/azure-costs/`. A future session will look for an export and not find one. `minReplicas: 0` additionally makes the idle rate structurally unreachable, and idle is **CPU-only** (memory bills the same either way), so idle is ~3.5× cheaper, not ~8.5× |
...
| ⛔ **SUPERSEDED 2026-08-15 by the row below — read that first.** The diagnosis here survives; the framing ("a reading ~14 points too high"), the headroom figure, and "zero OOM kills" do not | `get_container_memory_percent` (`utils.py:457`, ours) subtracts `inactive_file` only — correct as a working-set definition, and its docstring sized that at *1 % warm / 16 % cold*. Under 15 h of sustained crawling the cache is **active**, not inactive: at 420 refusal events the guard read **88.7 %** while `anon` was **68.6 %**, with **583 MB of active file cache** charged in. **Only 4 of 420 refusals had `anon` itself ≥85 %.** ⛔ **"Zero OOM kills, zero exit 137" is false and was false when written** — the final sweep had **8 labelled OOM incidents** (first 2026-08-13, i.e. on the *large* fleet) and **74 container restarts**; see the OOM-labelling row below. Two things follow. **`max_browsers` is not the fix** — `config.yml:171-177` says so outright and the refusal log shows `resident=4/6`, under the cap, refused on percentage alone. And **p50/p95 memory are biased upward**: the janitor samples every 10 s above 80 % and 60 s below 60 %, oversampling high states ~6× (measured overstatement 5.3×). **`max` is unbiased and so is the refusal count; the percentiles are not** |
| **The guard's threshold is drawn through the middle of its own signal — so the refusal count measures nothing about memory** | Measured 2026-08-14/15 over 11 h and ~6,000 `Pool:` samples at 2 replicas: the reading's **p25 82 %, p50 86.5 %, p75 90 %**, against `memory_threshold_percent: 85.0`. The guard therefore reads **above its own trip point ~58 % of every hour, all night**, while true `anon` is **flat** (p50 ~2,900 MB of 4,096, unchanged across 12 hours). ~1,000 refusals/night is a **threshold-crossing count on a signal centred on the threshold**: a ~1-point distributional wobble swings it tens of percent. **Both repos then read a trend into it** — our "hourly climb 5.9 → 9.2 per 100 admits" and MAS's "68.7 per 1,000, outside the 40–63 band" are the same artefact. **Count refusals; never interpret them, never size memory pressure from them, and do not treat a change in the rate as a change in the system.** The fix this implies is not "read a better number" but "stop putting the threshold where the signal lives" |
| **And it can still OOM — at ordinary readings, with no warning in any sampled series** | **First confirmed `OOMKilled` / exit 137 in the service's history: 2026-08-15 04:30**, two containers, preceded by 3× `ContainerBackOff`. Self-recovered in seconds; **zero pages lost** (61 × 200 / 9 × 429 around the first restart, 56 × 200 around the second, no 500s, no ingress-only requests). In the 90 s before it, `anon` was **2,835–3,445 MB** and `mem` **80.5–93.5 %** — the middle of the all-night band. **So the mechanism is memory and no sampled threshold predicts it**: the kill lands on a transient allocation between 10-second samples. Three consequences. (1) The pending fix — read `anon` instead of the cache-inclusive reading — **would raise the ceiling the pool may reach, in a system that demonstrably dies without warning**; it must not ship alone. (2) `max_browsers` 6 → 4 and `idle_ttl_sec` are the only candidates that bound the **allocation** rather than re-scale the **meter**, which makes them the strongest options, not the rejected ones. (3) Blast radius grew ~6×: an OOM used to remove 8 % of the fleet, at 2 replicas it removes **50 %** |
| **A container restarts without the replica changing, and `az containerapp replica list` cannot see it** | Four container restarts on 2026-08-14/15 (19:33, 21:11, 04:23 ×2 → the 04:30 OOM). The replica object survives, **`createdTime` never changes**, and three of the four carried **no `Reason_s` at all** — no `OOMKilled`, no `StoppingContainer`, no `SuccessfulRescale` pair. I reported "no crashes tonight" twice on the strength of the replica list before catching it. **Detect with `ContainerAppSystemLogs_CL \| where Reason_s == "ContainerStarted"` cross-checked against replica `createdTime`** — any start not within ~10 s of a replica creation is a silent restart; confirm with console `supervisord started with pid 1`. Ordinary scale churn produces the same line, so **look for the `SuccessfulRescale` scale-down/up pair before calling one a crash** |
| **A smaller fleet is MORE efficient per request — the over-provisioning was manufacturing its own load** | Trigger 6 → 12 took the fleet ~12 → **2** on identical traffic. Browser launches per admit **0.369 → 0.12**, pool hit rate **26.6 % → ~65 %**, total fleet CPU **3.44 → 1.55 cores**, p99 **−28 %**, p50 **+3.8 %** over 12 h. Mechanism: MAS sends a per-company browser identity, so at 12 replicas a company's ~4–6 pages scattered round-robin and **each replica paid its own cold Chromium launch**. This is `tasks/crawl-cost-is-idle-replicas-not-slow-renders.md` §4's loop running backwards, and it is the first direct measurement of it in either direction. **Slot utilisation 7.0 % → ~54 %; cost ~€89/day → ~€14.5/day**. ⛔ **Superseded by the full sweep, 2026-08-17 — the direction holds, three numbers do not.** Over 122 h vs 54 h of real traffic rather than 12 h: mean fleet **11.05 → 1.97**, replica-hours **1,344 → 106**, launches per admit **0.339 → 0.110**, cost per 1,000 requests **$3.28 → $0.51 (6.4×)**. Slot utilisation is **8.9 % → 40 %**, not 7 → 54 — the earlier figure used `sum(RequestDuration)`, which **includes admission queue wait**; the honest instrument is the per-render `[COMPLETE] ⏱` line, which starts after the gate. **And p99 −28 % did not survive**: over the full regimes it is **p99 +67 %**, p50 +3.2 %. The 12-hour readout predated a change in MAS's input list, so that improvement is unattributable. Costs are **USD** |
| **Size the harm BEFORE opening the investigation — this is the expensive lesson of six months** | The final sweep (186,178 requests) let MAS price every defect on our open list in the only unit that matters, **captures they lost**: our 429s cost **29**, our 500s **156**, `render_defect` **27**, `unrenderable_content` **4** — against 60,874 attempts, of which **86.1 % stored** and **95 % of all failures were the origin**. Meanwhile the memory-guard family consumed weeks of sessions across two repos and its total cost was **29 captures**. **The single highest-value change in six months was one config number** (ACA trigger 6 → 12, 6.4× on cost), found in a week. **If you cannot state an upper bound on what a defect is costing, that bound is the first measurement** — this repo has consistently investigated first and sized second. Seven task files were closed on 2026-08-17 on exactly this reasoning (`tasks/done/post-sweep-closure-2026-08-17.md`) |
| **`contains` is TERM-based on `_CL` text columns, and it produced a false refutation of a correct claim** | Measured 2026-08-17: `Log_s contains "CONSENT STRUCTURAL"` and `Log_s has "…"` both returned **383**; `contains_cs` and `indexof(Log_s, "…") >= 0` both returned **0**. All 383 were ordinary `CONSENT DECLINED … structural=False` lines — both words present, separately. The true count is **0 in 186,178 requests**, so **the 120-named-selector census holds at 6,800× segment 2's sample**. It only bites when both words of a multi-word token appear separately elsewhere, which is why single-emission tokens check out identically (`RenderGate ADMIT` 184,404 under both). **Use `indexof(col, "…") >= 0` for every multi-word token.** Worst direction possible: it manufactures a refutation of something true |
| **ACA's `OOMKilled` label is roughly a 1-in-5 sample of real memory kills, and exit 137 has three causes** | ACA emits `OOMKilled` **only** for a repeat kill inside the kubelet crash-backoff window. Measured over the sweep with zero overlap: the **7 labelled** kills came **1.2–6.8 min** after the previous boot of that replica; the **27 unlabelled** restarts came **median 178 min** after, **none under 10**. **An isolated memory kill produces no platform event of any kind** — no `OOMKilled`, no `StoppingContainer`, no rescale pair. The complete instrument is console `Log_s has "supervisord started with pid"`, differenced against each replica's first boot; `az monitor metrics list --metric RestartCount --interval PT1M --aggregation Total` agrees **exactly** (74 restarts: 40 + 34). Two further traps: **2 of 18 `OOMKilled` rows carried exit code `0`** (scale-downs mislabelled during a node event — parse the exit code out of `Log_s`), and **exit 137 also means SIGKILL after the 30 s scale-to-zero grace, or ephemeral-storage eviction at the 8 GiB cap** — and because we set `--disable-dev-shm-usage`, Chromium's shared memory lands in `/tmp`, which is that same budget. **Nothing watches it and it has the identical signature to an OOM** |
| **The ACA HTTP scaler is self-referential, and that is why a 2× trigger change moved the fleet 4.7×** | A `FailedComputeMetricsReplicas` error on 2026-08-11 named all eight metrics behind the scale rule: **`s{0,1}-upstream_{rq,cx}_{total,active}`** — two scalers (`activator`, `http-scaler`) × four Envoy metrics, **two of them connection counts**. `upstream_*` is Envoy→replica, so the metric is **replica-proportional by construction**. Fitting `metric = a·R + b·L` against **1,370 individual scaling decisions within one regime** gives `a ≈ 5.2`, loop gain `a/trigger` ≈ **0.87 at trigger 6** (near instability) vs **~0.45 at trigger 12** — and it predicts runaway at trigger ≤ 4–5, matching the recorded trigger-2 plateau of 38 replicas. Microsoft's published definition (*"requests in the past 15 seconds divided by 15"*) is off by **73–234×** on our data and cannot explain a single hour. **No public source documents any of this; our recovery of the metric names appears to be net-new.** An ACA engineer stated the connection half publicly in January 2023 and it never reached the docs |
| **Two independent instruments now say there is no memory leak, and they agree to the decimal** | Over MAS's 53.4 h window our `anon` p50 moved **2,932 → 2,958 MB of 4,096 (+26 MB, +0.6 %)**; their gauge-at-refusal moved **+0.13 points**. And their **4,455 refusal readings reproduce our 4,485 refusal log lines to one decimal on every percentile** (p25 87.0 / p50 89.2 / p75 91.6 / p95 95.4 / max 99.4, and both count exactly **30** reads at 85.0). The ~4-point overshoot they inferred as a "cached gauge" is nothing of the kind — `get_container_memory_percent()` is read fresh per call, and the overshoot is simply **the distribution conditional on being above a threshold that sits below its own median** (unconditional p50 **86.6 %**, 60.8 % of samples already above the trip point). **Their instrument is better than ours** for this: our janitor picks its sampling interval from the reading it just took, so it oversamples high states; theirs is triggered by request arrivals and is unbiased |
...
1. **Minimal upstream changes** — keep modifications isolated, document in `AITOSOFT_CHANGES.md`.
   **This is a maintenance argument, not an aesthetic one, and 2026-08-17 put numbers on it:** our
   fork is **+4,696 / −193 lines across 25 files**, of which ~2,143 sit in our own `aitosoft_*.py`
   files and cost nothing, while **~2,375 sit on 17 files upstream also owns**. Upstream is
   currently editing `api.py` and `server.py` — our two heaviest diffs — to solve the same problem
   class our failure taxonomy solves. **A merged PR deletes our divergence permanently; an unfiled
   one is a merge conflict every release, forever.**

   **Corollary, and it is the cheapest decision rule we have: do not build what upstream will ship
   in a month or two.** They see every user's experience; we see one client. Before starting
   anything in `crawl4ai/`, check `git log HEAD..upstream/develop` and the open issue/PR list —
   on 2026-08-17 that check found **five** problems we were carrying already fixed or in review
   upstream (PDF→500, `pypdf` missing, the `base_config` merge trap, cgroup v2 `"max"`, and the
   30 s body-visibility wait). Spend our effort on what we must hand-roll: the Azure deployment,
   the admission and pool layer, the failure taxonomy. Take theirs everywhere else.
...
7. **Reach for the cheapest lever before the cleverest one.** Our characteristic
   ...
   **Two additions from the 2026-08-17 review, both earned the hard way:**
   ...
   **Size the harm before opening the investigation.** If you cannot state an upper bound on what a
   ...
   first and sized second, and the cost of that only became visible when MAS priced our whole open
   list in captures: the memory-guard family consumed weeks across two repos and cost **29 captures
   of 60,874**. Meanwhile the single highest-value change in six months was **one config number**.

   **Research what is true *this month*, not six months ago.** This field moves fast and we have
   been burned in both directions. The 2026-08-17 pass found that the cookie-consent problem was
   solved by the wider ecosystem in a way we had not noticed (**every maintained system — autoconsent,
   uBO, EasyList, AdGuard, Ghostery — hides rather than removes; autoconsent's rule syntax has no
   `remove()` action at all**), *and* that our own operational knowledge of ACA exceeds anything
   published anywhere. Both mattered; neither was derivable from inside the repo. Do this
...
- `CLAUDE.md`, `AITOSOFT_CHANGES.md`, `AITOSOFT_FILES.md`, `AZURE_OPERATIONS.md`
...
| `AZURE_OPERATIONS.md` | Endpoint, credentials, Azure resource details |
...
- ⛔ **Configuration values are NOT written here any more, deliberately.** Image, revision, replica
  bounds, scale trigger, CPU and memory all used to be transcribed into this file and into
  `AZURE_OPERATIONS.md`, and a 2026-08-17 audit found **every one of them stale in at least one
  copy** — with the copies disagreeing, so whichever a session read first decided its action.
  **`AZURE_OPERATIONS.md` opens with the four `az` commands that answer them live.** Use those.
  The rule, so it does not get re-decided: *if `az` can answer it in one call, write the call, not
  the number.* Configuration is queried; behaviour and measurements are recorded.
- **What is worth knowing that `az` cannot tell you:** the scale rule's trigger is **not** the same
  quantity as `render_capacity` (different units — that category error cost weeks), `maxReplicas` is
  no longer a lever (peak was 19 of 45 at trigger 6, and **3** at trigger 12), and the environment
  is legacy Consumption-only so **2 vCPU / 4 GiB is its hard maximum** (2026-08-02, tested — and
  Microsoft's own docs contradict each other on this, so our `az` rejection is the tiebreak).
  ⛔ **The escape route was mis-priced by our own documentation for two weeks:** a workload-profiles
  (v2) environment's built-in **Consumption profile** allows **4 vCPU / 8 GiB at the identical meter
  rate, with scale-to-zero intact and no management fee** — management fees apply only to
  *Dedicated* profiles. The per-replica cost is 2× because the resources are 2×, not because the
  billing model changed. Migration is not in-place (new environment, new FQDN, a MAS-visible
  contract change), and **before any of that, Microsoft's FAQ says memory-limit overrides are
  "evaluated on a per-case basis" — one support ticket may remove the need entirely and nobody has
  tried it.** We do not need this today; it is recorded so the option stops being priced wrongly.
- **Auth:** Bearer token via `CRAWL4AI_API_TOKEN` env var
- See `AZURE_OPERATIONS.md` for full details
...
Provisioning reference (scale rule, probes, env vars): `AZURE_OPERATIONS.md`.
