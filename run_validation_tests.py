        ...
        try:
            ...
            if result == 0:
                ...
                if attempt < max_attempts - 1:
                    print_status(
                        f"Server not ready, retrying in {delay}s... (attempt {attempt + 1}/{max_attempts})"
                    )
                    time.sleep(delay)
            ...
            if attempt < max_attempts - 1:
                print_status(
                    f"Connection failed, retrying in {delay}s... (attempt {attempt + 1}/{max_attempts})"
                )
                time.sleep(delay)

    print_error(
        f"Server at {host}:{port} is not responding after {max_attempts} attempts"
    )
    return False
...
def main():
    ...
    parser.add_argument(
        "--local-only", action="store_true", help="Run only local tests"
    )
    parser.add_argument(
        "--production-only", action="store_true", help="Run only production tests"
    )
    parser.add_argument("--production-url", help="Production URL for testing")
    parser.add_argument(
        "--bearer-token",
    ...
    if not args.local_only and args.production_url:
        production_results = run_production_tests(
            args.production_url, args.bearer_token
        )
    ...
    success = print_summary(
        local_results, production_results if production_results else None
    )
