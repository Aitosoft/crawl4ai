...
from simple_auth import verify_api_token
...
async def authenticated_crawl(
    request: CrawlRequest, auth: dict = Depends(verify_api_token), req: Request = None
):
    ...
    try:
        ...
        result["auth"] = {"user": auth.get("user", "unknown"), "authenticated": True}
